                  ...
                  <?php
                    ...
                    if ($_FILES['form-file']['size']) {
                      ...
                      for ($i=0; $i<$countFiles;$i++) {
                              $uploadfile = $uploaddir . basename($_FILES['form-file']['name'][$i]);
                            if (move_uploaded_file($_FILES['form-file']['tmp_name'][$i], $uploadfile)) {
                              var_dump('Файл корректен и был успешно загружен .\n'); 
                          } else {
                            var_dump('Проблема с загрузкой файла.\n');
                          }
                          $userLogin = $_SESSION['login'];
                          var_dump('логин= '.$userLogin);
                          $result = $mysqli->query("SELECT id FROM users WHERE login='".$userLogin."'");
                          $zapros = "SELECT id FROM users WHERE login='".$userLogin."'";
                          var_dump('запрос= '.$zapros);
                          $arr = $result->fetch_assoc(); 
                          $id = $arr['id'];
                          var_dump('айди= '.$id);
                          if($mysqli->query("INSERT INTO files ('id', 'user_id', 'file_dir', 'info') VALUES (null,'".$id."','".$uploadfile."','".$info."')")) {
                            var_dump('запрос отправлен');
                          }
                      }
